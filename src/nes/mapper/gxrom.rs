//! `GxROM` (mapper 66) — 32 KB PRG + 8 KB CHR bank switching.
//!
//! - $8000–$FFFF: switchable 32 KB PRG bank
//! - CHR-ROM: switchable 8 KB bank
//! - Mirroring: static (from cartridge header)
//!
//! Write to $8000–$FFFF: bits 4–5 = PRG bank, bits 0–1 = CHR bank.
//!
//! Used by: Super Mario Bros. / Duck Hunt combo cart, Dragon Power.
...
/// `GxROM` mapper — simple 32 KB PRG + 8 KB CHR switching.
#[derive(Clone)]
...
impl Mapper for Gxrom {
    ...
    fn cpu_write(&mut self, addr: u16, val: u8) {
        if addr >= 0x8000 {
            self.prg_bank = ((val >> 4) & 0x03) % self.prg_bank_count;
            self.chr_bank = (val & 0x03) % self.chr_bank_count;
        }
    }
